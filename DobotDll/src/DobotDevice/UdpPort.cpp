...
bool UdpPort::open(OpenMode mode)
{
    ...
    int _tempPort(12345);
    while(1) {
        bool res = m_udpSocket->bind(QHostAddress(QHostAddress::AnyIPv4), _tempPort);
        if (res) {
            //qDebug() << "m_udpSocket bind success!" << _tempPort;
            break;
        }
        else {
            //qDebug() << "m_udpSocket bind failed!" << _tempPort;
            ++_tempPort;
        }
